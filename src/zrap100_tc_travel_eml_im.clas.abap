"! @testing BDEF:ZR_RAP100_ATRAV_IM
CLASS zrap100_tc_travel_eml_im DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  FOR TESTING
...
CLASS zrap100_tc_travel_eml_im IMPLEMENTATION.
  METHOD class_setup.
    " create the test doubles for the underlying CDS entities
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                      i_for_entities = VALUE #(
                        ( i_for_entity = 'ZR_RAP100_ATRAV_IM' ) ) ).
  ...
  METHOD create_with_action.
    ...
    MODIFY ENTITIES OF zr_rap100_atrav_im
     ENTITY Travel
    ...
    SELECT * FROM zr_rap100_atrav_im INTO TABLE @DATA(lt_travel). "#EC CI_NOWHERE
